...
#LOAD
df_tree<-read.csv("~/Desktop/Masters/Classes/Machine Learning/Projects/Evergreen - stumbleupon/Phase 3/train.csv")
str(df_tree)
...
error_nb = sum(pred_nb != test_nb$label)/nrow(test_nb)

########################### Bagging: Random Forest ###############################

#Import
df_rf<-read.csv("~/Desktop/Masters/Classes/Machine Learning/Projects/Evergreen - stumbleupon/Data/train.csv")

#Manage
df_rf$alchemy_category<-as.factor(df_rf$alchemy_category)
df_rf$is_news<-as.factor(df_rf$is_news)
df_rf$lengthyLinkDomain<-as.factor(df_rf$lengthyLinkDomain)
df_rf$news_front_page<-as.factor(df_rf$news_front_page)
df_rf$hasDomainLink<-as.factor(df_rf$hasDomainLink)
df_rf$label<-as.factor(df_rf$label)
df_rf$framebased<-as.factor(df_rf$framebased)

df_rf$url<-NULL
df_rf$urlid<-NULL
df_rf$boilerplate<-NULL
df_rf$alchemy_category_score<-NULL
...
model_rf<-randomForest(label ~., data=training, ntree=750)
...
pred_rf_full<-predict(model_rf,df_rf)
#input 3: Naive bayes
pred_nb_full<-predict(model_nb, df_nb)

##Step 2
df_stack<-cbind(df, pred_glm_full, pred_rf_full, pred_nb_full)
...
#Stacked Model
model_stack<-naiveBayes(label ~., data = train_stack)
