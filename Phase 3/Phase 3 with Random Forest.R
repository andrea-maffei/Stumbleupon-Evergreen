...
#Manage
df$alchemy_category<-as.factor(df_rf$alchemy_category)
df$is_news<-as.factor(df_rf$is_news)
df$lengthyLinkDomain<-as.factor(df_rf$lengthyLinkDomain)
df$news_front_page<-as.factor(df_rf$news_front_page)
df$hasDomainLink<-as.factor(df_rf$hasDomainLink)
df$label<-as.factor(df_rf$label)
df$framebased<-as.factor(df_rf$framebased)
...
##Step 2
df_stack<-cbind(df, pred_glm_full, pred_tree_full, pred_nb_full)
