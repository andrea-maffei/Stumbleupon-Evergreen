...
##Step 1#Getting a clean df in with all the standard insertions
...
##Step 2
df_stack<-cbind(df, pred_glm_full, pred_tree_full)

##Step 3
N_stack<- nrow(df_stack) 
training_size_stack<-round(N_stack*0.6) 
train_stack<-sample(N_stack, training_size_stack) 
train_stack<-df_stack[train_stack,]
test_stack<-df_stack[-train_stack,]
